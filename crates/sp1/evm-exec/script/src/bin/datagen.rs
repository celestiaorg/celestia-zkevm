...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for block_number in start_height..(start_height + num_blocks) {
        ...
        let mut extended_header = celestia_client.header_get_by_height(block_number).await?;
        extended_header.header.version.app = 3; // Required until celestia rust libs support v4
