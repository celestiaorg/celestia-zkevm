//! A simple utility to extract and print the verifier key for the `ev-range-exec-program` zkVM circuit.
//!
...
//! ```shell
//! cargo run -p ev-range-exec-script --bin vkey --release
//! ```
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = "elfs/ev-range-exec-elf.bin";
