    ...
    for block_number in start_height..=(start_height + num_blocks) {
        let handle = tokio::spawn({
            ...
            async move {
                ...
                let proof_data = bincode::serialize(&proof.proof).expect("Failed to serialize proof");
                let public_values = proof.public_values.to_vec();
                let output: ev_zkevm_types::programs::block::BlockExecOutput =
                    bincode::deserialize(proof.public_values.as_slice()).expect("Failed to deserialize public values");
